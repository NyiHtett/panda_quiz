...
const Page = () => {
    ...
    const onAnswerSelected = (answer, index) => {
        ...
        if(answer != correctAnswer) {
            ...
            setChecked(false)
            setSelectedAnswer(0);
            if ((activeQuestion+1) == dataForName.totalQuestions) {
                setFinish(true);
            }
            else {
    ...
    return (
        <div className='container'>
            {!(finish) ? (
                <div>
                <h1>Guess Item Name</h1>
                <div>
                <h2> Question: {activeQuestion + 1}<span>/{dataForName.totalQuestions}</span></h2>
            </div>
                {!showResult ? (
